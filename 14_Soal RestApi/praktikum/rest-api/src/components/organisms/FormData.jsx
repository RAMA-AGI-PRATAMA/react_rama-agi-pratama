...
const FormData = () => {
  ...
  const getItems = async () => {
      try{
          const response = await axios.get('https://6536104cc620ba9358ecf11f.mockapi.io/Product');
  ...
  const handleSubmit = async (e) => {
    ...
    if(Object.keys(errors).length === 0){
      ...
      if(duplicate){
        ...
        try{
          const response = await axios.post('https://6536104cc620ba9358ecf11f.mockapi.io/Product', formValues)
  ...
  const handleDeleteItem = async (id) => {
    if(window.confirm('Apakah Anda yakin ingin menghapus item ini?')){
      try{
        axios.delete(`https://6536104cc620ba9358ecf11f.mockapi.io/Product/${id}`);
